...
def create_engine(device_type: str, epochs: int, save_checkpoints: bool) -> Engine:
    """Create the appropriate engine based on a device type.

    Args:
        device_type: Either 'gpu', 'xpu', or 'cpu'
        epochs: Number of training epochs
        save_checkpoints: If True, model checkpoints will be saved during benchmarking.

    Returns:
        Configured Engine instance
    """
    if not save_checkpoints:
        model_checkpoint_cb = ModelCheckpoint(save_top_k=0)  # Save topk=0 means no checkpoints are saved
        logger.info("Setting save_top_k=0 for ModelCheckpoint to avoid saving checkpoints during benchmarking.")
    else:
        model_checkpoint_cb = None  # Uses anomalib engine default callbacks
        logger.info("Model checkpoints will be saved after every epoch during benchmarking.")

    callbacks = [model_checkpoint_cb] if model_checkpoint_cb else None
    ...
    return Engine(max_epochs=epochs, callbacks=callbacks)
...
def benchmark_model(
    ...
    try:
        ...
        # Create engine
        engine = create_engine(device_type, epochs, save_checkpoints=save_checkpoints)
...
def main():
    ...
    parser.add_argument(
        "--save-checkpoints",
        action="store_true",
        help="If set, model checkpoints will be saved during benchmarking.",
    )
