...
class ViTill(nn.Module):
    ...
    def generate_mask(self, feature_size, device="cuda"):
        ...
        if self.remove_class_token:
            return mask
        mask_all = torch.ones(
            h * w + 1 + self.encoder.num_register_tokens, h * w + 1 + self.encoder.num_register_tokens, device=device
        )
...
class BottleNeckMLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0, grad=1.0):
        super().__init__()
